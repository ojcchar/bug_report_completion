...
public class SVMBugReport {
	...
	public void readSource() {
		this.paragraphs = new ArrayList<String>();
		this.xmlLine = this.xmlLine.replace("<description>", "<description><![CDATA[")
				.replace("</description>", "]]></description>").replace("<title>", "<title><![CDATA[")
				.replace("</title>", "]]></title>");
	...
	public void preprocessing() {
		...
		for (int pid = 0; pid < this.paragraphs.size(); pid++) {
			...
			// flag used to indicate whether we split paragraph by row, to check
			// if there exists some special case
			boolean flag = false;
			...
			// corenlp
			// if((errorflag==true || report.paragraphs.get(pid).length()>700)
			// && flag==false){
			// for(String s:ss){
			...
			for (int id = 0; id < parseSentences.size(); id++) {
				...
				// if(parseSentence.startsWith("java.")||parseSentence.startsWith("
				// at ")||parseSentence.startsWith("\"")
				// ||parseSentence.startsWith("org.apache")){//parseSentence.matches("(\\W*)(at
				// )(\\w+\\.){3,}(.*)")){
				...
				for (CoreMap s : sentence) {
					...
					for (CoreLabel token : tokens) {
						// String word = token.word().replace("``",
						// "\"").replace("''", "\"").replace("`",
						// "'").replace("-LRB-", "(").replace(" ", " ")
						...
						// System.out.print(token.word()+"/"+token.originalText()+"
						// "+token.tag()+" ");

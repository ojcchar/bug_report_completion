...
class CRF(nn.Module):
    ...
    def __init__(self, tagset_size, gpu, device):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        # mask = 1 + (-1)*mask
        mask =  (1 - mask.long()).byte() # (seq_len, batch_sizes)
        #print(mask)
        _, inivalues = next(seq_iter)  # bat_size * from_target_size * to_target_size
        # only need start from start_tag
        ...
        decode_idx[-1] = pointer.view(decode_idx[-1].size()) #change from v0.3 to v0.4
        for idx in range(len(back_points)-2, -1, -1):
            pointer = torch.gather(back_points[idx], 1, pointer.contiguous().view(batch_size,1))
            #print("pointer:",pointer)
            
            #pointer = pointer.view(decode_idx[idx].size())
            #print(pointer)
            #input(" ")

            #print(pointer.size())
            #print(decode_idx[idx].size())

            #print(pointer)
            #print(decode_idx[idx])

            decode_idx[idx] = pointer.view(decode_idx[idx].size()) #change from v0.3 to v0.4
            #print("decode:",decode_idx)
            #input(" ")

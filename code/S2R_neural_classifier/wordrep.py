...
class WordRep(nn.Module):
    def __init__(self, data):
        ...
        if self.use_char:
            ...
            if data.char_feature_extractor == "CNN":
                self.char_feature = CharCNN(data.char_alphabet.size(), data.pretrain_char_embedding, self.char_embedding_dim, self.char_hidden_dim, data.HP_dropout, self.gpu, data.device).to(data.device)
            #elif data.char_feature_extractor == "LSTM":
            #    self.char_feature = CharBiLSTM(data.char_alphabet.size(), data.pretrain_char_embedding, self.char_embedding_dim, self.char_hidden_dim, data.HP_dropout, self.gpu, data.device)
            #elif data.char_feature_extractor == "GRU":
            #    self.char_feature = CharBiGRU(data.char_alphabet.size(), data.pretrain_char_embedding, self.char_embedding_dim, self.char_hidden_dim, data.HP_dropout, self.gpu, data.device)
        ...
        if self.gpu:
            ...
            self.word_embedding = self.word_embedding.to(data.device)
            #for idx in range(self.feature_num):
    ...
    def forward(self, word_inputs,feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        ...
        if self.use_char:
            ...
            #print("char_featurs size:",char_features.size())

...
public class ErrorTermSubjectPM extends ObservedBehaviorPatternMatcher{
	...
	public int matchSentence(Sentence sentence) throws Exception {
		...
		for(int i=0;i<tokens.size();i++) {
			...
			if ((Arrays.stream(VerbErrorPM.ERROR_TERMS).anyMatch(t -> token.getLemma().contains(t))
					&& token.getGeneralPos().equals("NN"))||(Arrays.stream(NegativeAdjOrAdvPM.NEGATIVE_ADJ).anyMatch(t -> token.getLemma().contains(t))
							&& token.getGeneralPos().equals("JJ"))||(Arrays.stream(NegativeAdjOrAdvPM.NEGATIVE_ADV).anyMatch(t -> token.getLemma().contains(t))
									&& token.getGeneralPos().equals("RB"))||compoundErrorTerms(tokens)||NounsAsVerbs(tokens)){
	...
	public boolean compoundErrorTerms(List<Token> tokens){
		for(int i=0;i<(tokens.size()-1);i++){
			Token token = tokens.get(i);
			if(token.getGeneralPos().equals("NN")&&token.getLemma().equals("blank")&&tokens.get(i+1).getGeneralPos().equals("NN")&&tokens.get(i+1).getLemma().equals("page")){
				return true;
			}
		}
		return false;

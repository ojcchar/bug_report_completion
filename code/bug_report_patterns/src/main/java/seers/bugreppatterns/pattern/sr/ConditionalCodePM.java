		...
		for (Sentence subordinate : subordinates) {
			...
			if (!conditionalPositions.isEmpty()) {
				...
				for (int i = startingIndex; i < clauses.size(); i++) {
					if (clauses.get(i).getTokens().stream().anyMatch(t -> {
						// Attempt to split the lemma for stuff like files with
						// extensions
						String[] components = t.getLemma().split("\\.");

						return Stream.of(components).anyMatch(CodeRefPM.NOUNS_TERM::contains);
					})) {
						return 1;
					}
				}

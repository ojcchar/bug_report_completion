...
public class SimplePastPM extends StepsToReproducePatternMatcher {
    /**
     * Verbs whose present and simple past forms are identical and are assumed to be past giving
     * them the benefit of the doubt.
     */
    private static final Set VERBS_ASSUMED_PAST = JavaUtils.getSet("set");

    @Override
    public int matchSentence(Sentence sentence) throws Exception {
        List<Token> tokens = sentence.getTokens();

        // Times we have assumed a verb with identical present and past forms to be past.
        int assumptions = 0;

        // Times we can tell with a good probability that a verb is acceptable for this pattern.
        int nonAssumptions = 0;

        for (int i = 0; i < tokens.size(); i++) {
            Token token = tokens.get(i);

            // If we find a verb we have to establish whether it is acceptable for the pattern
            if (token.getGeneralPos().equals("VB")) {
                if (VERBS_ASSUMED_PAST.contains(token.getLemma())) {
                    // When there's ambiguity, we assume the verb is in past
                    assumptions++;
                } else if (token.getPos().equals("VBD") ||
                        isTrueInfinitive(sentence, i) ||
                        isMistaggedSimplePast(sentence, i) ||
                        isMistaggedNoun(sentence, i) ||
                        isGerund(sentence, i)) {
                    /**
                     * Acceptable verbs:
                     * - Simple past (VBD)
                     * - Tagged as participle (VBN) but not after a "to have" or "to be"
                     *   (likely VBD)
                     * - Tagged present (VBP) but are likely to be nouns (are after a CC)
                     * - Tagged gerund (VBG) but not present participle (not preceded by "to be")
                     */
                    nonAssumptions++;
                } else {
                    // If the verb has any other form, the sentence doesn't match
                    return 0;
                }
            }
        }
    ...
    /**
     * Returns true if the token in the specified position is tagged as VBN but is not after a
     * "to have" or "to be" verb, since in this case it is most likely a VBD.
     *
     * @param sentence Sentence to search in.
     * @param position position of the token suspected to be VBD.

...
public class CodeRefPM extends StepsToReproducePatternMatcher{
	...
	final static String[] NOUNS_TERM = {"snippet","code","sample","configuration","statement","script","html/ssi","html/fbml","html","trace","test.htm"};
	...
	public int matchSentence(Sentence sentence) throws Exception {
		...
		for(int i=0;i<tokens.size();i++){
			...
			}else if(Arrays.stream(ADV_LOCATION).anyMatch(t -> token.getLemma().equals(t))&&((token.getGeneralPos().equals("RB"))||token.getGeneralPos().equals("IN"))){
				isAdverb=true;
			}else if(Arrays.stream(VERB_DEMO).anyMatch(t -> token.getLemma().equals(t))&&(token.getGeneralPos().equals("VB"))){

    ...
    public int matchSentence(Sentence sentence) throws Exception {
        ...
        for (i = 0; i < clauses.size(); i++) {
            ...
            if (!SentenceUtils.isImperativeSentence(clause)) {
                // If the clause is not imperative there is only one case in which it will be
                // accepted as part of this pattern: if it is at the beginning and contains no verbs
                List<Token> tokens = clause.getTokens();
                if (i != 0 ||
                        tokens.stream().anyMatch(t -> t.getGeneralPos().equals("VB"))) {
                    break;
                }
            } else {
                imperativeSentences++;
        ...
        // If there aren't any imperative sentences at the beginning, there is no match
        if (imperativeSentences == 0) {
            return 0;
        }

        // If we find anything that is not observed behavior, but we accept if it contains no verbs

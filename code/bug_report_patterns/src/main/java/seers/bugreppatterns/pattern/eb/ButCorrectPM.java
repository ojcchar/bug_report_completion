...
public class ButCorrectPM extends ExpectedBehaviorPatternMatcher {
	...
	public int matchSentence(Sentence sentence) throws Exception {
		List<Integer> contrastTermIndexes = SentenceUtils.findLemmasInTokens(CONTRAST_TERMS, sentence.getTokens());
		if (!contrastTermIndexes.isEmpty()) {
			List<Sentence> subSentences = SentenceUtils.findSubSentences(sentence, contrastTermIndexes);
			...
			for (int i = 0; i < subSentences.size() - 1; i++) {
				...
				if (negAuxMatcher.matchSentence(firstSentence) == 0
						&& !SentenceUtils.findLemmasInTokens(POSITIVE_TERMS, secondSentence.getTokens()).isEmpty()) {
					return 1;
				}
			}
		}

...
public class CauseNounAdjectivePM extends ObservedBehaviorPatternMatcher{
	...
	private int indexVerbTokens(List<Token> tokens){
		for(int i=0;i<tokens.size();i++){
			...
			if(token.getGeneralPos().equals("VB")&&Arrays.stream(INDEX_VERBS).anyMatch(t -> t.equals(token.getLemma()))){
				return i;
			}
		}
		return 0;
	...
	private boolean isNegativeNounPhrase(Sentence sentence){
		...
		for(int i=0; i<tokens.size();i++){
			Token token = tokens.get(i);
			if(Arrays.stream(VerbErrorPM.ERROR_TERMS).anyMatch(t->t.equals(token.getLemma()))&&token.getGeneralPos().equals("NN")){
				return true;
			}else if(Arrays.stream(NegativeAdjOrAdvPM.NEGATIVE_ADJ).anyMatch(t -> t.equals(token.getLemma()))&& token.getGeneralPos().equals("JJ")){
				return true;
			}else if(Arrays.stream(NegativeAdjOrAdvPM.NEGATIVE_ADV).anyMatch(t -> token.getLemma().contains(t))&&token.getGeneralPos().equals("RB")){
				return true;
			}
		}
		return false;

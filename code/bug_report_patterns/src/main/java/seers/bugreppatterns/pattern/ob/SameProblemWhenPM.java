...
public class SameProblemWhenPM extends ObservedBehaviorPatternMatcher {
	...
	// final public String [] CONDITIONAL_CLAUSE = {"when", "if", "upon",
	// "while", "whenever", "every time", "whereas"};
	final public String[] LOC_AND_TEMP_PREP = { "in", "on", "at", "since",
			"for", "before", "after", "from", "until", "till", "by", "beside",
			"under", "below", "over", "above", "across", "through", "into",
			"toward", "onto" , "of", "with"};
			...
			for (Integer index_term : index_terms) {
				if(index_term == (index_sim+1)){
					...
					for (int i =0; i< toks.size(); i++){
						...
						if(tok.getGeneralPos().equals("VB") && tok.getLemma().equals("be") && ((index_sim == i+1) || (index_sim == i+2))){
							return 1;
						}
					}
				}
	...
	public List<Integer> findTerms (List<Token> tokens){
		...
		for (int i = 0; i<tokens.size(); i++){
			...
			if(token.getGeneralPos().equals("NN") && (Arrays.stream(terms).anyMatch(t-> token.getLemma().equals(t)))){
				termsList.add(i);
			}
		}
		return termsList;
	...
	public boolean checkClause(List<Token> tokens) {
		...
		for (int i = 0; i < tokens.size(); i++) {
			Token token = tokens.get(i);
			if (Arrays.stream(CONDITIONAL_TERMS).anyMatch(
					t -> token.getLemma().equals(t)) || (Arrays.stream(LOC_AND_TEMP_PREP).anyMatch(
							t -> token.getLemma().equals(t)))) {
				return true;
			}
		}
		return false;
...
/*	public int findIndexEpression(List<Token> tokens) {
		...
		for (int i = 1; i < tokens.size(); i++) {
			...
			if ((token.getGeneralPos().equals("NN"))
					&& (Arrays.stream(terms).anyMatch(t -> token.getLemma()
							.equals(t)))

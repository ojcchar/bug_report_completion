...
public class LabeledListPM extends StepsToReproducePatternMatcher {
	...
	public int matchParagraph(Paragraph paragraph) throws Exception {
		...
		if (sentences.size() > 1) {
			for (int i = 0; (i < sentences.size() - 1); i++) {
				String text = TextProcessor.getStringFromLemmas(sentences
						.get(i));
				boolean b = (text.matches("(?s).*(following|repro) step.*"))
						|| (text.matches("(?s).*?(step)? ?to repro.*"))
						|| (text.equals("step :"))
						|| (text.equals("str :"))
						|| (text.endsWith("have try :"))
						|| (text.contains("here be the step") || (text
								.matches("(?s).*reproduce as follow :")));
				if (b) {
					isLabeled = true;
				}
			}
	...
	public boolean isAnAction(Token firstToken, Token secondToken) {
		...
		if (firstToken.getGeneralPos().equals("VB") || firstToken.getPos().equals("VBP")||firstToken.getGeneralPos().equals("MD")) {
			return true;
		} else {
			if (secondToken != null) {
	...
	public boolean startsWithNounPhrase(Token first, Token second){
		...
		if((first.getGeneralPos().equals("JJ")&&second.getGeneralPos().equals("NN"))||(first.getGeneralPos().equals("NN")&&second.getGeneralPos().equals("JJ"))){
			return true;
		}
		return false;
	}
